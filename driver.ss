...
(define-syntax (define-language-wrapper x)
  (define (wrapper-exception-handler name)
    (lambda (c)
      (cond
        [(warning? c) (display-condition c)]
        [else (raise (condition (make-wrapper-violation 'name) c))])))
  (syntax-case x (environment)
    [(_ (n1 n2 ...) (args ...) exps ...)
     #'(begin
    ...
    [(_ name (args ...) (environment env) exps ...)
     #'(define name
         (let ([the-env env])
           (lambda (args ...)
             (with-exception-handler (wrapper-exception-handler name)
               (lambda ()
